...
public class Refrain.Audio.Song : Object {
    ...
    public static Song insert (Album album, File file, string name, int track) throws DBError {
        ...
        return new Song.from_file (file);
    }
    ...
    public Song.from_file (File file) throws DBError {
        unowned var db = DB.get_default ().get_db ();
        string query = "SELECT * FROM song WHERE path = ?";
        ...
        if (song_id == "") {
            throw new DBError.NOT_FOUND ("song not found with path %s", file.get_path ());
        }
